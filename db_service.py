...
class SQLiteDBHandler:
    ...
    • viewed  – уже отправленные объявления (id+price).
    • settings – KV‑pairs пользовательских параметров.
    • searches – активные задачи (слепок настроек хранится JSON‑ом).
    • scan_history - история сканирований (для сравнения новых объявлений)
    • cian_viewed - просмотренные объявления ЦИАН
    • cian_scan_history - история сканирований ЦИАН
    """
    ...
    def _migrate_database(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Проверяем наличие столбца name в таблице searches
            c.execute("PRAGMA table_info(searches)")
            columns = [column[1] for column in c.fetchall()]
            
            # Добавляем столбец name, если его нет
            if "name" not in columns:
                c.execute("ALTER TABLE searches ADD COLUMN name TEXT")
                conn.commit()
            
            # Проверяем наличие столбца platform в JSON настройках
            ...
            for row in rows:
                ...
                if settings_json:
                    try:
                        ...
                        # Если нет поля platform, добавляем его со значением 'avito' (по умолчанию)
    ...
    def add_search(self, user_id: int, platform: str, urls: List[str], settings: Dict[str, Any], name: str = "") -> int:
        # Добавляем платформу в настройки
        settings_copy = settings.copy()
        settings_copy["platform"] = platform
        
        with sqlite3.connect(self.db_path) as conn:
            # Получаем ВСЕ ID из таблицы searches, а не только активные
            cursor = conn.cursor()
            all_ids = cursor.execute("SELECT id FROM searches").fetchall()
            all_ids = [row[0] for row in all_ids]
            
            # Найдем первое свободное число, начиная с 1
            new_id = 1
    ...
    def list_active_searches(self, user_id: int | None = None, platform: str | None = None) -> List[Tuple]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Добавляем столбец name в запрос, если он существует
            if "name" in columns:
                sql += ", name"
            else:
                sql += ", ''" # Возвращаем пустую строку, если столбца нет
